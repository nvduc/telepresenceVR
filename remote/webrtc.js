...
var default_peer_id = 2222;
var other_peer_id = 1111;
...
function onServerMessage(event) {
    ...
    switch (event.data) {
        case "HELLO":
            ...
            // starting session
            ws_conn.send('SESSION ' + other_peer_id);
            return;
        default:
...
function getLocalStream() {
    ...
    var localstream = getDeviceStream(options);
    return localstream;
}
...
const handleDataChannelOpen = (event) =>{
    ...
    const sender = peer_connection.getSenders()[1];
    const parameters = sender.getParameters();
    console.log("#####" + sender);
    console.log(parameters);
    const maxBR = parameters.encodings[0].maxBitrate;
    console.log(maxBR);
              ...
              if (report.type === 'outbound-rtp') {
                ...
                if (lastResult && lastResult.has(report.id)) {
                  ...
                  const headerrate = 8 * (headerBytes - lastResult.get(report.id).headerBytesSent) /
                    (now - lastResult.get(report.id).timestamp);
                  console.log("Send bitrate:", bitrate);
